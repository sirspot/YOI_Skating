...
var zim = function(zim) {
	...
	zim.Physics = function(gravity, borders, scroll, frame) {
		...
		if (zot(frame) && (typeof zdf == "undefined" || zot(zdf))) {console.log("zim.Physics() - please provide a zim Frame object"); return;}
		frame = frame || zdf;
		that.frame = frame;
		...
		this.noDrag = function(list) {
			...
			if (!zot(list)) {
				...
				for (var i=0; i<list.length; i++) {
					if (list[i].body) list[i] = list[i].body;
					var index = this.dragList.indexOf(list[i]);
					if (index>=0) this.dragList.splice(index, 1);
				}
			} else {
		...
		this.Drag = function(list) {
			...
			this.update = function() {
				...
				if(isMouseDown && (!mouseJoint)) {
					...
					if(body) {
						if (that.dragList.length > 0 && that.dragList.indexOf(body) < 0) return;
